...
module.exports = class planilhaExcelController{
  ...
  static async excelPlanilha(req,res){
    ...
    let segundos = tempoAgora.getSeconds()
    console.log(segundos)

    let excelFileName = `C:/Users/luizl/OneDrive/Área de Trabalho/${user}.xlsx`;
    let somador = 0
    while(true){
        if(fs.existsSync(excelFileName)){
            console.log(somador, 'somadoer')
            somador++
        }else{
            console.log('break')
            break
        }
        excelFileName = `C:/Users/luizl/OneDrive/Área de Trabalho/${user}${somador}.xlsx`
    }
